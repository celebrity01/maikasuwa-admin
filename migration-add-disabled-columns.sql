-- KASUWA 2.0 — Migration: Add is_disabled and default_password_set columns
-- Run this in the Supabase SQL Editor (https://supabase.com/dashboard → SQL Editor)
-- Also run the full schema in supabase-schema.sql if tables don't exist yet.

-- ═══════════════════════════════════════════════════
-- STEP 1: Create seller_profiles table if it doesn't exist
-- ═══════════════════════════════════════════════════
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE table_schema = 'public' AND table_name = 'seller_profiles' AND column_name = 'is_disabled'
  ) THEN
    ALTER TABLE public.seller_profiles ADD COLUMN is_disabled BOOLEAN DEFAULT false;
  END IF;
END $$;

-- Add default_password_set column if missing
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_schema = 'public' AND table_name = 'seller_profiles' AND column_name = 'default_password_set'
  ) THEN
    ALTER TABLE public.seller_profiles ADD COLUMN default_password_set BOOLEAN DEFAULT false;
  END IF;
END $$;
...
DROP POLICY IF EXISTS "Allow public insert" ON public.seller_profiles;
...
DROP POLICY IF EXISTS "Service role full access" ON public.seller_profiles;
...
DROP POLICY IF EXISTS "Allow public read active" ON public.products;
